...
// Extracts a readable message from axios/backend errors
function getErrorMessage(error: unknown): string {
  if (axios.isAxiosError(error)) {
    const data = error.response?.data;
    // Common backend shapes: { message }, { error }, { errors: [...] }
  ...
  const onSubmit = async (data: ClientFormData) => {
    const handlers = {
      ...
      onError: (error: unknown) => {
        ...
        // Surface field-level errors inline if the backend tells us which field
        if (axios.isAxiosError(error)) {
